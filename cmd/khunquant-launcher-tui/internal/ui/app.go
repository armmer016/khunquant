...
func Run() error {
	...
	if cfg == nil {
		cfg = khunquantconfig.DefaultConfig()
	}
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open khunquant agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("khunquant", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("khunquant", "gateway")
...
func (s *appState) isActiveModelValid() bool {
	...
	hasKey := strings.TrimSpace(cfg.APIKey.String()) != "" || strings.TrimSpace(cfg.AuthMethod) == "oauth"
