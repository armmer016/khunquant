...
func (c *OpenClawConfig) ConvertToKhunQuant(sourceHome string) (*KhunQuantConfig, []string, error) {
	cfg := &KhunQuantConfig{}
	var warnings []string
	...
	if c.HasSkills() {
		warnings = append(
			warnings,
			fmt.Sprintf(
				"Skills (%d entries) not automatically migrated - reinstall via khunquant CLI",
				len(c.Skills.Entries),
			),
		)
	}
	if c.HasMemory() {
		warnings = append(warnings, "Memory backend config not migrated - KhunQuant uses SQLite with vector embeddings")
	}
	if c.HasCron() {
		warnings = append(
			warnings,
			"Cron job scheduling not supported in KhunQuant - consider using external schedulers",
		)
	}
	if c.HasHooks() {
		warnings = append(warnings, "Webhook hooks not supported in KhunQuant - use event system instead")
	}
	if c.HasSession() {
		warnings = append(warnings, "Session scope config differs - KhunQuant uses per-agent sessions by default")
	}
...
func (c *OpenClawConfig) convertChannels(warnings *[]string) ChannelsConfig {
	...
	if c.Channels.Signal != nil {
		*warnings = append(*warnings, "Channel 'signal': No KhunQuant adapter available")
	}
	if c.Channels.IRC != nil {
		*warnings = append(*warnings, "Channel 'irc': No KhunQuant adapter available")
	}
	if c.Channels.Mattermost != nil {
		*warnings = append(*warnings, "Channel 'mattermost': No KhunQuant adapter available")
	}
	...
	if c.Channels.BlueBubbles != nil {
		*warnings = append(
			*warnings,
			"Channel 'bluebubbles': No KhunQuant adapter available - consider iMessage instead",
		)
	}
...
func (c *KhunQuantConfig) ToStandardConfig() *config.Config {
...
func (c ChannelsConfig) ToStandardChannels() config.ChannelsConfig {
	return config.ChannelsConfig{
		...
		Telegram: config.TelegramConfig{
			...
			Token:   *config.NewSecureString(c.Telegram.Token),
			Proxy:   c.Telegram.Proxy,
		},
		Feishu: config.FeishuConfig{
			Enabled:           c.Feishu.Enabled,
			AppID:             c.Feishu.AppID,
			AppSecret:         *config.NewSecureString(c.Feishu.AppSecret),
			EncryptKey:        *config.NewSecureString(c.Feishu.EncryptKey),
			VerificationToken: *config.NewSecureString(c.Feishu.VerificationToken),
		},
		Discord: config.DiscordConfig{
			Enabled:     c.Discord.Enabled,
			Token:       *config.NewSecureString(c.Discord.Token),
		...
		QQ: config.QQConfig{
			...
			AppID:     c.QQ.AppID,
			AppSecret: *config.NewSecureString(c.QQ.AppSecret),
		},
		DingTalk: config.DingTalkConfig{
			Enabled:      c.DingTalk.Enabled,
			ClientID:     c.DingTalk.ClientID,
			ClientSecret: *config.NewSecureString(c.DingTalk.ClientSecret),
		},
		Slack: config.SlackConfig{
			Enabled:  c.Slack.Enabled,
			BotToken: *config.NewSecureString(c.Slack.BotToken),
			AppToken: *config.NewSecureString(c.Slack.AppToken),
		},
		Matrix: config.MatrixConfig{
			Enabled:      c.Matrix.Enabled,
			Homeserver:   c.Matrix.Homeserver,
			UserID:       c.Matrix.UserID,
			AccessToken:  *config.NewSecureString(c.Matrix.AccessToken),
			AllowFrom:    c.Matrix.AllowFrom,
			JoinOnInvite: true,
		},
		LINE: config.LINEConfig{
			Enabled:            c.LINE.Enabled,
			ChannelSecret:      *config.NewSecureString(c.LINE.ChannelSecret),
			ChannelAccessToken: *config.NewSecureString(c.LINE.ChannelAccessToken),
